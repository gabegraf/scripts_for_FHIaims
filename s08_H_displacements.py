...
# NOTE: The Minimum Image Convention (MIC) function has given me inaccurate numbers in the past.
#       I cannot figure out why. Default to supercell method. 
        ...
        for y in [-1, 0, 1]:
            for z in [-1, 0, 1]:
                for atom in atomic_coords:
                    ...
                    shift_vector = np.dot(x, lattice_vectors[0]) + np.dot(y, lattice_vectors[1]) + np.dot(z, lattice_vectors[2])
...
def calculate_H_displacement(initial_geometry, final_geometry):
    
    displacements = { "C-H": [], "N-H": []}

    for i, (atom_1, atom_2) in enumerate(zip(initial_geometry, final_geometry)):
        if atom_1[4] == atom_2[4] and atom_1[4] == "H":
            displacement = np.linalg.norm(np.array(atom_2[1:4], dtype=np.float64) - np.array(atom_1[1:4], dtype=np.float64))
    ...
    with open("displacements_and_bond_length_differences.out", 'w') as file:
    ...
    print(f"H atom displacement data written to H_displacements.out")
...
def calculate_bond_length_changes(initial_geometry_cleaned, final_geometry_cleaned):
    ...
    with open("displacements_and_bond_length_differences.out", 'a') as file:
...
calculate_H_displacement(initial_geometry_cleaned, final_geometry_cleaned)
